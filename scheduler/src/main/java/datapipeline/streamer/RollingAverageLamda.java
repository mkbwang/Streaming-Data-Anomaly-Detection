...
public class RollingAverageLamda {
    ...
    public RollingAverageLamda(int ID, int threshold, String topic, int window) {
        _ID = ID;
                ...
                .flatMap((k, v) -> {
                    ...
                    for (int i = 0; i < value.length; i++) {
                        for (int j = 0; j < value[i].length; j++) {
                            result.add(new KeyValue<>(Integer.toString(i * value[i].length + j), value[i][j] + ";" + k));
                        }
                    }
                    return result;
                });
