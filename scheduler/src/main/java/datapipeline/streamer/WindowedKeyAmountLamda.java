...
public class WindowedKeyAmountLamda implements Runnable{

    private AnomalyDetectionJob _job;

    public WindowedKeyAmountLamda(AnomalyDetectionJob job) {
        this._job = job;
    }
    ...
    public void run() {
        ...
        KafkaManager.deleteTopicWithPrefix(KafkaConstants.STREAMER_PREFIX + _job.get_ID());
        ...
        KGroupedStream<String, String> groupedStream = views.groupByKey(Grouped.with(stringSerde, stringSerde));
        ...
        KTable<Windowed<String>, Long> aggregatedStream = windowedStream
                .count(Materialized.with(stringSerde, Serdes.Long()))
                .suppress(Suppressed.untilWindowCloses(Suppressed.BufferConfig.unbounded()));

        KStream<String, String> anomalousData = aggregatedStream
                //.filter((key, value) -> value > (double)_job.get_threshold())
                .toStream()
        ...
        wrapper.getControlledStream(anomalousData
                , builder
                , KafkaConstants.CONTROL_FLOW_TOPIC_PREFIX + _job.get_ID()
                , new ControlledFilterTransformer<>(_job.get_ID(), _job.get_threshold())
                ,_job.get_ID())
                .to(KafkaConstants.STREAMER_OUTPUT_TOPIC_PREFIX + _job.get_ID());
